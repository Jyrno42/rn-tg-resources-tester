...
const expectedBuffer = Buffer.from([0x62, 0x75, 0x66, 0x66, 0x65, 0x72]);
...
function configureServer(logger) {
    const app = express();
...
const listen = (p = port, logger = false) => configureServer(logger).listen(p);

const getHostUrl = (p = port) => `http://127.0.0.1:${p}`;
